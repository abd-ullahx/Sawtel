		...
		<div class="card">
		    <div class="card-header d-flex align-items-center justify-content-between">
				...
				<div>
					...
					<a href="{{ route('support_tickets.accept', $supportticket->uuid) }}" class="btn btn-primary btn-xs"><i class="fas fa-check-circle"></i> {{ _lang('Accept') }}</a>	
					@endif

					@if($supportticket->status == 1)
					<a href="{{ route('support_tickets.mark_as_closed', $supportticket->uuid) }}" class="btn btn-danger btn-xs"><i class="fas fa-check-circle"></i> {{ _lang('Mark as Closed') }}</a>
					@endif

					@if($supportticket->status == 2 && $supportticket->is_resolved == 0)
					<a href="{{ route('support_tickets.mark_as_resolved', $supportticket->uuid) }}" class="btn btn-success btn-xs"><i class="fas fa-check-circle"></i> {{ _lang('Mark as Resolved') }}</a>
					@endif
				</div>
			</div>
			...
			<div class="card-body comment-box">
				...
				<form method="post" action="{{ route('support_tickets.reply', $supportticket->uuid) }}" enctype="multipart/form-data">
					@csrf
					<div class="row">
						<div class="col-lg-12">
						...
						<div class="col-lg-12 mt-3">
							<div class="text-right">
								<button class="btn btn-outline-primary" type="submit"><i class="fas fa-reply"></i> {{ _lang('Reply Message') }}</button>
							</div>
						</div>
					</div>
				</form>
