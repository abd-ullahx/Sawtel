...
<div id="invoice" class="{{ $type }}">
	<div class="custom-invoice">
		...
		$invoice_content = str_replace('<!--$invoice_items_header-->', view('backend.admin.quotation.template.components.invoice-items-header', compact('quotation', 'quotationColumns'))->render(), $invoice_content);
		$invoice_content = str_replace('<!--$invoice_items-->', view('backend.admin.quotation.template.components.invoice-items', compact('quotation', 'quotationColumns'))->render(), $invoice_content);
		$invoice_content = str_replace('<!--$invoice_summary-->', view('backend.admin.quotation.template.components.invoice-summary', compact('quotation'))->render(), $invoice_content);
		$invoice_content = str_replace('$tax_loop', view('backend.admin.quotation.template.components.invoice-taxes', compact('quotation'))->render(), $invoice_content);
